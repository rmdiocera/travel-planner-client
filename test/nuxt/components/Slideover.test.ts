...
describe('Slideover.vue', () => {
  ...
  describe('Slots rendering', () => {
    it('renders content passed to the form slot', async () => {
      const { bodyWrapper } = await mountSlideover(
        {
          props: { open: true },
          slots: {
  ...
  describe('Events', () => {
    it('emits update:open when closed', async () => {
      const { wrapper, bodyWrapper } = await mountSlideover({ props: { open: true } })
